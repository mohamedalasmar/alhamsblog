...
if($_POST){
    ...
    if(empty($tag) > 0){
        $error = "خلل في اضافة الوسم";
    }else{
        $query = "insert into tags (name, user_id) 
        values('$tag', $user_id)";
        
        if(mysqli_query($conn, $query)){
            $message = "تم إضافة الوسم بنجاح";
        }else{
            $error = "خلل في اضافة الوسم  ".mysqli_error($conn);
        }
    }
}
?>
<section class="download section-padding" style="height: 500px !important; ">
      ...
      <section class="explain section-padding">
        ...
        <form action="#" method="post" enctype="multipart/form-data">
                        <h2 style="text-align:center;">إضافة وسم</h2>
                        <div class="name">
                           <input type="text" name="tag" placeholder="اسم الوسم المراد اضافته" required> 
                        </div>
                        <br>
                        <div class="submit" style="text-align:center;">
                           <button class="form-submit" name="submit">إضافة وسم</button>
                        </div>
                        </div>
                        </div>
                        </div>
