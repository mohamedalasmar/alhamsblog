...
if($_POST){
    ...
    $id = $_GET['id'];
    if(empty($tag) > 0){
        $error = "خلل في تعديل الوسم";
    }else{
        $query = "update tags set name='$tag' where id =$id";
    } 
        if(mysqli_query($conn, $query)){
            $message = "تم تعديل الوسم بنجاح";
        }else{
            $error = "خلل في تعديل الوسم  ".mysqli_error($conn);
        }
        if($_GET){
...
<section class="download section-padding" style="height: 500px !important; ">
      ...
      <section class="explain section-padding">
        ...
        <form action="#" method="post" enctype="multipart/form-data">
                        <h2 style="text-align:center;">تعديل وسم</h2>
                        <div class="name">
                           <input type="text" name="tag" placeholder="اسم الوسم المراد اضافته" required value="<?php if(isset($row['tag'])) echo $row['tag'] ; ?>"> 
                        </div>
                        <br>
                        <div class="submit" style="text-align:center;">
                           <button class="form-submit" name="submit">حفظ التغيرات</button>
                        </div>
                        </div>
                        </div>
                        </div>
